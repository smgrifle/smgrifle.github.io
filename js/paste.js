...
function submitComment(token = null){
	if(captcha == 'hcaptcha'){
	...
	}else if(captcha == 'turnstile'){ // AFTER AN HOUR OF NOT REALIZING BRO HAHAHAHAHHAHA
		var username = "Anonymous";
		var comment = $('#comment').val();
		...
		$.post("/comment", {
			...
			comment: comment,
			_token: $('input[name=_token]').val(),
			turnstile_token: $('[name=cf-turnstile-response]').val(),
		}, function(resp){
			if(resp.status == "done"){
				username = htmlspecialchars(username.trim());
				comment = htmlspecialchars(comment.trim());
...
$(document).ready(function(){
	$('#cancel-edit-request').on('click', function(e){
        $.post('/api/paste/cancel-edit-request', {
            ...
            $('#edit-paste-list a.btn').text('Edit Paste').css('cursor', 'pointer').attr('href', '/edit/' + pastelink).attr('title', null);
			$('#edit-info').remove();
			$('#edit-paste-list a.btn').after('<p id="edit-info" style="color: #cccccc; text-align: center;margin: 0;">' + resp.msg + '</p>');
        });
        e.preventDefault();
	});
